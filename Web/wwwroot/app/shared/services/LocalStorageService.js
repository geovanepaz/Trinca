...
(function ()
{
   ...
   * LocalStorage.list();
   * LocalStorage.exists('propertyName');
   *
   * @version   1.0
   *
   */
  ...
  function LocalStorageService($window, $rootScope)
  {
     ...
     */
    const storage = (typeof window.localStorage === "undefined") ? undefined : window.localStorage;
    ...
    * $digest cicle so all values are refreshed in the view
    ...
    /**
     * Verify property is stored on localStorage
     *
     * @return {boolean}
     */
    function exists(property)
    {
      ...
      return $window.localStorage && $window.localStorage.hasOwnProperty(property);
    }
